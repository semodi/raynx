...
def test_simple_graph_foreach():
    ...
    results.clear()
    graph.run(simple_in, SimpleContext())
    assert results[0].dict() == {"list_float": [1.22, 1.32], "single_float": 2.42, "logical": True}
...
def test_simple_graph_reuse():
    ...
    results.clear()
    graph.run(simple_in, SimpleContext())
    expected_out = SimpleOutput(list_float=[1.31, 1.41, 0.01], single_float=2.51, logical=True)
...
def test_simple_graph_kwargs():
    ...
    results.clear()
    graph.run(simple_in, SimpleContext())
    expected_out = SimpleOutput(list_float=[1.21, 1.31, -0.09], single_float=2.41, logical=True)
    assert results[-1].dict() == expected_out.dict()
