...
def out_to_in(output_model: SimpleOutputIncompatibleConverted) -> SimpleInput:
    return SimpleInput(list_float = output_model.list_float, 
                       logical=output_model.logical,
                       single_float=2.3)

simple_in = SimpleInput(list_float=[1.1, 1.2, -0.2], single_float=2.3, logical=False)
simple_out = SimpleOutputCompatible(
    list_float=[1.1, 1.2, -0.2], single_float=2.3, logical=False
)
simple_out_incomp = SimpleOutputIncompatible(list_float=[1.1, 1.2, -0.2], logical=False)
simple_out_incomp_conv = SimpleOutputIncompatibleConverted(list_float=[1.1, 1.2, -0.2], logical=False)
iter_in = IterInput(
...
def test_input_iter():
    ...
    model_iter = iter_in.iter_fields(
        ["list_float1", "list_float2"], mode="outer", batch_size=1
    )
    first = next(model_iter)
    assert first.list_float1 == [1.1]
    assert first.list_float2 == [2.1]
    ...
    model_iter = iter_in.iter_fields(
        ["list_float1", "list_float2"], mode="outer", batch_size=2
    )
    first = next(model_iter)
    ...
    with pytest.raises(RuntimeError):
        next(
            iter_in.iter_fields(
                ["list_float1", "list_float2", "list_float_short"], batch_size=2
            )
        )
